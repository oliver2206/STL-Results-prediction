...
export default function App() {
  ...
  // requires between two of a combo's draws — selectable 6-12, defaults 6.
  ...
  const everySixMonthsData = useMemo(() => {
    ...
    for (const [normalized, matches] of byCombo) {
      ...
      let gapMatches = 0
      for (let i = 1; i < sorted.length; i++) {
        if (calendarMonthGap(sorted[i - 1].date, sorted[i].date) === sixMonthEverGap) gapMatches++
      }
      if (gapMatches < 1) continue
    ...
    return { combos, fromKey: SIX_MONTH_EVER_FROM_KEY, toKey }
  }, [allDraws, sixMonthEverGap])
  ...
  const randomPool = useMemo(() => {
    ...
    if (randomStrategy === 'month') {
      ...
      for (const [normalized, matches] of byCombo) {
        ...
        if (inScope.length >= randomMinTimes) results.push(finish(normalized, inScope))
      }
      return results
    }
  ...
  return (
    <div className="page">
        ...
        <div className="mode-toggle">
          <button
            ...
            className={`mode-toggle-btn ${
              !compareMode && !predictMode && !myNumberMode && !cycleMode && !yearlyMode && !hotColdMode && !randomMode
                ? 'is-active'
                : ''
            }`}
            onClick={() => {
      ...
      {!isSearching && !compareMode && !predictMode && !myNumberMode && !hotColdMode && !randomMode && (
          ...
          <>
            ...
            <div className="cycle-group six-month-ever-group">
              ...
              <p className="predict-disclaimer six-month-ever-intro">
                {`Numbers that have landed twice with an exact ${sixMonthEverGap}-month gap somewhere between January 2024 and this month — each one's next window is projected from its most recent hit.`}
              </p>
